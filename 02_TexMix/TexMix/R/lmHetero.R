...
#'
#' @source The maximum likelihood estimation procedure for multiplicatively
...
#' mod.wlm <- lm(PERCAPINC~PCTNOHINS+PCTMINOR+PCTUNIVDEG+PCTWHITE, weights=mod.lmH1$weights,
#'               data=validTractShp)
#' summary(mod.wlm)
...
lmHetero <- function(formula, hetero=~1, data, subset, na.action,
  ...
  ################################################################################################
  ##
  ## Purpose: Calculate multiplicatively weighted regression models with respect to externally
  ##          given exogenous variables and weights variables
  ##
  ...
  ##
  ##     !!! Important user input: weight variables Z must be enter log-transformed !!!
  ##
  ## Authors: Michael Tiefelsdorf (tiefelsdorf@utdallas.edu) & Yongwan Chun
  ###############################################################################################
  ...
  #require(Formula)
    ...
    stopifnot(length(formula)[1] == 1L, length(formula)[2] %in%
                1:2)
    has_dot <- function(formula) inherits(try(stats::terms(formula),
                                              silent = TRUE), "try-error")
    if (has_dot(formula)) {
      f1 <- formula(formula, rhs = 1)
      f2 <- formula(formula, lhs = 0, rhs = 2)
      if (!has_dot(f1) & has_dot(f2))
        formula <- Formula::as.Formula(f1, stats::update(formula(formula,
                                                                 lhs = 0, rhs = 1), f2))
    }
    mf$formula <- formula
    ...
    if (length(formula)[2] < 2L) {
      ...
      mtZ <- stats::delete.response(stats::terms(formula, data = data,
                                                 rhs = 2, ...))
      Z <- stats::model.matrix(mtZ, mf, contrasts)
      zNames <- colnames(Z)
      if (!all(exp(Z) > 0))
        stop("All weight variables must be positive!")
    }
    nofreg <- length(y)
    ...
    logLikeH1 <- -(nofreg/2) * log(2 * pi) - 0.5 * sum(log(Sigma2[,
                                                                  1])) - 0.5 * sum(res2/Sigma2[, 1])
