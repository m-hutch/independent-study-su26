...
test_that("mij values are flattened correctly when provided", {
  ...
  out <- flow2vec(df, mij = as.data.frame(mij))
...
test_that("df must be a data frame", {
  expect_error(flow2vec(list(a = 1:3)))
})

test_that("logTrans must be logical", {
  expect_error(flow2vec(make_small_df(), logTrans = "yes"))
})

test_that("zTrans must be logical", {
  expect_error(flow2vec(make_small_df(), zTrans = 1))
})
